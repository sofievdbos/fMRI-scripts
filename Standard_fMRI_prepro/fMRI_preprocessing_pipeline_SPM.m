%% fMRI (rs or task) standard preprocessing pipeline using SPM (BIDS folder format) created on MATLAB_R2015b (http://bids.neuroimaging.io/).
...
for i=1:length(sbj_fold)
    ...
    BIDS_rename(anat_targetdir(i,1), func_targetdir(i,1), '*.nii', 'f4D*.nii', 'task', sbj_fold(i).name)
