...
%           - line 35-36: Change to your source (dicomdir/) and target (my_dataset/) path.
%           - line 52-63: Change some arguments in the preprocessing steps. Check the REMARKs in the subscripts.
%
...
source = '/media/sofie/Sofie_Disk4/Maaike_kids/2015/01_Ima_data/'; sourcedir = ([source 'dicomdir/']); %source folder
target = '/media/sofie/Sofie_Disk4/Maaike_kids/2015/02_Pre_processing/SPM/P3/allbefore1scfix_discarded/'; targetdir = ([target 'my_dataset/']); %target folder
sbj_fold=dir([sourcedir 'sub*']); st
...
parfor i=1:length(sbj_fold)
    ...
    normalize(anat_targetdir(i,1), func_targetdir(i,1), '^y_c.*\.*', '^rof4D.*\.*', [3 3 3], 4, sbj_fold(i).name)
    smooth(func_targetdir(i,1), '^wrof4D.*\.*', [8 8 8], sbj_fold(i).name) 
    normalize_struct(anat_targetdir(i,1), '^y_.*\.*', '^c.*\.*', '^c1.*\.*', '^c2.*\.*', '^c3.*\.*', '^c4.*\.*', '^c5.*\.*', [1 1 1], 4, sbj_fold(i).name) 
